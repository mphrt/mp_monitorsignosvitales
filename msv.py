...
# ========= Pie de página (según PDF MONITORES_V2) =========
FOOTER_LINES = [
...
def draw_si_no_boxes(pdf, x, y, selected, size=4.5, gap=4,
                     text_gap=1.5, label_w=36):
...
def create_checkbox_table(pdf, section_title, items, x_pos, item_w, col_w,
    ...
    pdf.cell(item_w, row_h, f"{title_prefix}{section_title}",
             border=1, ln=0, align="L", fill=True)
    ...
    for item, value in items:
        ...
        pdf.cell(max(1, item_w - indent_w), row_h, item,
                 border=0, ln=0, align="L")
        pdf.cell(col_w, row_h, "X" if value == "OK" else "",
                 border=1, ln=0, align="C")
        pdf.cell(col_w, row_h, "X" if value == "NO" else "",
                 border=1, ln=0, align="C")
        pdf.cell(col_w, row_h, "X" if value == "N/A" else "",
...
def draw_analisis_columns(pdf, x_start, y_start, col_w, data_list):
    ...
    def draw_column_no_lines(x, y, data):
        ...
        def field(lbl, val=""):
            ...
            pdf.cell(label_w, row_h_field, f"{TAB}{lbl}",
                     border=0, ln=0)
            pdf.set_xy(x + label_w + 2, yy)
            pdf.cell(text_w, row_h_field, f": {val}",
                     border=0, ln=1)
            yy += row_h_field
    ...
    if num_equipos >= 3:
        ...
        end_right_row2 = 0
        if num_equipos >= 4:
            end_right_row2 = draw_column_no_lines(right_x, y_current,
                                                  data_list[3])
...
def main():
    ...
    def add_equipo():
        if len(st.session_state.analisis_equipos) < 4:
    ...
    if st.button("Generar PDF"):
        ...
        LOGO_W_MM = 60
        sep = 4
        title_text = "PAUTA MANTENCIÓN MONITOR MULTIPARAMETRO"
        ...
        pdf.cell(cell_w, title_h, title_text, border=1, ln=1,
                 align="C", fill=True)
        ...
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", "B", 7.5)
        pdf.cell(col_total_w, 4.0,
                 f"{TAB}5. Instrumentos de análisis",
        ...
        draw_si_no_boxes(pdf, x=SECOND_COL_LEFT, y=y_equipo_op,
                         selected=operativo, size=4.5, label_w=40)
        pdf.ln(1.6)
        ...
        add_signature_inline(pdf, canvas_result_tecnico,
                             x=x_sig_tecnico, y=y_nombre,
                             w_mm=65, h_mm=20)
        ...
        draw_boxed_text_auto(
            ...
            w=col_total_w,
            min_h=10,
            title=f"{TAB}Observaciones (uso interno)",
        ...
        st.download_button(
            "Descargar PDF",
            out,
            file_name=f"MP_Monitor_Multiparametro_{sn}.pdf",
